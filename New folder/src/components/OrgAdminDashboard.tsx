...
type ActiveTab = 'home' | 'branches' | 'staff' | 'config' | 'loan-scheme-config' | 'reports' | 'holidays' | 'operations' | 'master-roll';
  ...
  if (selectedSimulationStaff) {
    return (
      <div className="min-h-screen bg-slate-50 flex flex-col font-sans">
        {/* Prominent Admin Simulation Warning Banner */}
        <div className="bg-rose-700 bg-gradient-to-r from-rose-600 via-amber-600 to-rose-600 text-white py-3 px-4 shadow-xl flex flex-col sm:flex-row justify-between items-center gap-3 text-xs sm:text-xs font-bold z-50 animate-in fade-in duration-200">
          <div className="flex items-center gap-2">
            <span className="p-1 px-1.5 bg-white/25 rounded-lg text-white text-[10px]">⚠️ অ্যাডমিন কন্ট্রোল</span>
            <div>
              আপনি বর্তমানে <strong className="underline text-yellow-200 font-extrabold">{selectedSimulationStaff.name} ({selectedSimulationStaff.designation || 'মাঠ কর্মী'})</strong> হিসেবে <strong className="font-extrabold text-blue-100">{branchesList.find(b => b.id === selectedSimulationStaff.branchId)?.name || 'শাখা'}</strong> প্যানেলে কাজ করছেন। সকল ড্যাশবোর্ড ইন্টারফেস ও ট্রানজেকশন অ্যাডমিনের সাথে সমন্বয় হবে।
            </div>
          </div>
          <button
            type="button"
            onClick={() => setSelectedSimulationStaff(null)}
            className="bg-white text-rose-700 hover:bg-slate-50 border border-slate-200 shadow-sm px-4 py-1.5 rounded-xl font-extrabold transition active:scale-95 text-xs flex items-center gap-1 cursor-pointer shrink-0"
          >
            <ChevronLeft size={14} />
    ...
    <div className="min-h-screen bg-slate-50 flex flex-col font-sans">
      ...
      <div className="bg-indigo-900 bg-gradient-to-r from-blue-700 via-indigo-800 to-violet-900 text-white py-4 px-4 sm:px-6 shadow-xl z-20">
        <div className="max-w-7xl mx-auto space-y-4">
          ...
          <div className="flex flex-col sm:flex-row justify-between items-center gap-3 border-b border-white/10 pb-3">
            ...
            <div className="flex items-center flex-wrap gap-2 text-[11px] font-medium">
              <button 
                ...
                className="flex items-center gap-1.5 bg-white/10 hover:bg-white/20 px-3 py-1.5 rounded-lg border border-white/20 text-white transition-all cursor-pointer"
              >
          ...
          {/* Bottom Line: Org Details & Logged-in Person (Admin) Credentials */}
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-3 text-xs text-white/90">
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                <span className="text-[9px] text-white/60 font-bold block leading-none uppercase tracking-wider">প্রতিষ্ঠান</span>
            ...
            <div className="flex items-center gap-5 text-xs font-semibold text-white/90 border-t md:border-t-0 border-white/10 pt-3 md:pt-0 w-full justify-between md:justify-end">
              <div className="flex items-center gap-2">
                <span className="text-white/60">অ্যাডমিন আইডি:</span> 
                <span className="font-bold text-white tracking-wide font-mono bg-black/20 px-2 py-0.5 rounded">{org.adminId}</span>
              </div>
              ...
              <div className="flex items-center gap-3 border-l border-white/20 pl-5">
                ...
                <button 
                  onClick={onLogout}
                  className="px-3 py-1.5 bg-rose-600 hover:bg-rose-700 text-white rounded-lg transition-colors font-bold text-[10px] flex items-center gap-1.5"
                >
                  <LogOut size={11} />
                  <span>লগআউট</span>
                </button>
