...
import { LayoutDashboard, Users, CreditCard, LogOut, Loader, BarChart3, Database } from 'lucide-react';
...
export default function StaffDashboard({ org, staff, onLogout }: StaffDashboardProps) {
  ...
  useEffect(() => {
    ...
    const wDay = localStorage.getItem(`tanzil_working_day_${org.id}`) || '';
    ...
    <div className="min-h-screen bg-gray-50 p-4">
      <header className="flex justify-between items-center mb-6">
        ...
        <div className="flex gap-2">
            <button 
              onClick={() => setActiveView(v => v === 'dashboard' ? 'stats' : 'dashboard')}
              className={`flex items-center gap-2 px-4 py-2 ${activeView === 'stats' ? 'bg-blue-600 text-white' : 'bg-gray-100 text-gray-700'} rounded-lg hover:opacity-90`}
            >
              {activeView === 'stats' ? <LayoutDashboard size={18}/> : <BarChart3 size={18} />} {activeView === 'stats' ? 'ড্যাশবোর্ড' : 'পরিসংখ্যান'}
            </button>
            <button 
              onClick={onLogout}
